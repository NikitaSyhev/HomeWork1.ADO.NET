...
//001.(-) tbConnectionString.Text = ConnectionString; Справедливо, но при внесении изменений в поле, используется текстовая переменная ConnectionString.
//Нужно менять литерал в коде вручную, что будет недоступно пользователю. - не понял комментарий

//002.(+) Кнопка "Create DB".Создалась БД Library, пустая таблица Books.
//003.(-) Кнопка "Fill DB".Invalid object name 'Books'. В 69 стр. необходимо указать контекст выполнения запроса:  string fillDataBase = @"use Library;..." - USE LIBRARY добавил
//Использование кирилицы из кода программы может приводить к проблемам - скрин во вложении. Ошибкой это на считаю, просто, учтите на будущее. Проблема выявлена только у LocalDB.- про кириллицу принял

//004.(-) Кнопка "Поиск по автору".Invalid object name 'Books'. В 138 стр. необходимо указать контекст выполнения запроса:  string _Expression = @"use Library;... -  - USE LIBRARY добавил
//005.(-) Кнопка "Поиск по названию". Invalid object name 'Books'. В 108 стр. необходимо указать контекст выполнения запроса:  string _Expression = @"use Library; ... -  - USE LIBRARY добавил
//006.(-) Задание "Показывать самые популярные и самые не популярные книги." не реализовано. - реализовал
//Также при поиске неплохо выводить не только количество, но и что найдено и использовать не строгое совпадение, а оператор LIKE (SQL). - LIKE добавил и по нему комментарий принял, плюс сделал вывод
